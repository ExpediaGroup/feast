...
var (
	REPEATED_PROTO_VALUES = []*types.RepeatedValue{
		//{},
		{Val: []*types.Value{}},
		{Val: []*types.Value{nil_or_null_val}}, // Invalid
	...
	MULTIPLE_REPEATED_PROTO_VALUES = [][]*types.RepeatedValue{
		{
			...
			{Val: []*types.Value{&types.Value{}}}, // Empty Array
			nil,                                   // TODO: Check this. How Null values are represented
			//{},	// Invalid
		},
		{
			{Val: []*types.Value{{Val: &types.Value_Int32Val{Int32Val: 10}}, {Val: &types.Value_Int32Val{Int32Val: 20}}}},
		...
		},
		{
			...
			{Val: []*types.Value{{Val: &types.Value_Int32ListVal{Int32ListVal: &types.Int32List{Val: []int32{}}}}}}, // Invalid
			//{Val: []*types.Value{{}, {}}}, //Invalid
			{Val: []*types.Value{}},
			//{},
		},
		{
...
func protoRepeatedValueEquals(t *testing.T, a *types.RepeatedValue, b *types.RepeatedValue) {
	assert.Truef(t, proto.Equal(a, b),
		"Values are not equal. Diff %v != %v", a, b)
}
...
func TestRepeatedValueRoundTrip(t *testing.T) {
	...
	for i, repeatedValue := range REPEATED_PROTO_VALUES {
		...
		protoRepeatedValueEquals(t, repeatedValue, result[0])
...
func TestMultipleRepeatedValueRoundTrip(t *testing.T) {
	...
	for i, batch := range MULTIPLE_REPEATED_PROTO_VALUES {
		...
		//test_val, err := ArrowValuesToRepeatedProtoValuesDuplicate(arrowArray)
		//t.Log(test_val)
		...
		//
		//	if original.Val == nil {
		//		assert.Nil(t, result.Val, "Expected nil value")
		//		continue
		//	}
		//
