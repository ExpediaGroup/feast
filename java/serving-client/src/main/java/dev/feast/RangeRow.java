...
public class RangeRow {
    ...
    public RangeRow setEntityTimestamp(List<Instant> timestamps) {
        field_timestamps = timestamps.stream().map(t -> Timestamps.fromMillis(t.toEpochMilli())).collect(Collectors.toList());
        return this;
    }
    ...
    public RangeRow setEntityTimestamps(List<String> dateTimes) {
        field_timestamps = dateTimes.stream().map(dt -> Timestamps.fromMillis(Instant.parse(dt).toEpochMilli())).collect(Collectors.toList());
        return this;
    }
    ...
    public RangeRow set(String fieldName, List<Object> values, List<FieldStatus> statuses) {
        List<Value> valueList = values.stream().map(RequestUtil::objectToValue).collect(Collectors.toList());
    ...
    public List<byte[]> getBytes(String fieldName) {
        return getValues(fieldName).stream().map(Value::getBytesVal).map(ByteString::toByteArray).collect(Collectors.toList());
    }
